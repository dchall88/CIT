function vid_player( fName, dispFunc )
% Simple GUI to play video files.
%
...
%
% USAGE
%  vid_player( [fName], [dispFunc] )
%
% INPUTS
%  fName    - optional video file to load at start
...
if(~isempty(fName)), menuApi.vidOpen(fName); end
  ...
  function api = dispMakeApi()
    ...
    function setVid( sr1 )
      ...
      if(~isstruct(sr)), cla(pMid.hAx); else
        info=sr.getinfo(); nFrame=info.NumberOfFrames;
    ...
    function dispLoop()
      ...
      while( 1 )
        ...
        if( speed~=0 )
          ...
          del = speed * max(10,info.FrameRate) * min(.1,eTime);
        ...
        sr.seek( round(curInd)+1 ); I=sr.getframe();
        if(~isempty(hs)), delete(hs); hs=[]; end
        if(~isempty(dispFunc)), hs=dispFunc(round(curInd)); end
        assert(~isempty(I)); set(hImg,'CData',I);
        set(pMid.hSl,'Value',curInd); c=round(curInd/info.FrameRate);
    ...
    function setSpeed( speed1 )
      if((speed1>0&&curInd==nFrame)||(speed1<0&&curInd==0)),speed1=0;end
  ...
  function api = menuMakeApi()
    ...
    function updateMenus()
      ...
      set([m.hVidCls m.hVidInfo m.hVidAud],'Enable',en); nm='Video Player';
    ...
    function vidOpen( flag )
      ...
      if(all(ischar(flag)))
        [d,fn,ext]=fileparts(flag); if(isempty(d)), d='.'; end;
        d=[d '/']; f=[fn ext]; flag=1;
      elseif(iscell(flag)), assert(length(flag)==2);
        [d,fn,ext1]=fileparts(flag{1}); if(isempty(d)), d='.'; end;
        [d2,fn2,ext2]=fileparts(flag{2}); if(isempty(d2)), d2='.'; end;
        d=[d '/']; f=[fn ext1]; d2=[d2 '/']; f2=[fn2 ext2]; flag=2;
      elseif( flag==1 )
        [f,d]=uigetfile({'*.avi;*.mp4;*.mov','All Video Files (*.avi, *.mp4, *.mov)';...
          '*.*','All Files' },'Select video',d);
      elseif( flag==2 )
        [f,d]=uigetfile({'*.avi;*.mp4;*.mov','All Video Files (*.avi, *.mp4, *.mov)';...
          '*.*','All Files' },'Select first video',d);
        [f2,d2]=uigetfile({'*.avi;*.mp4;*.mov','All Video Files (*.avi, *.mp4, *.mov)';...
          '*.*','All Files' },'Select second video',d);
        if( f2==0 ), return; end
      end
      if( f==0 ), return; end; vidClose(); fVid=[d f];
      try
        if( flag==1 ), sr=vid_io(fVid,'r',2); else
          sr=vid_io({fVid,[d2 f2]},'rdual'); end
