...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
	...
	//double *ROI = (double*) mxGetData(prhs[6]);
	
	// get dimensions and constants
	const size_t numClfs = mxGetNumberOfElements (clfs);
	...
	// define variables
	vector<int> rs, cs, id; vector<float> hs1;

	for(int k=0; k<numClfs; k++){

		/*
			double xmin = ROI[0*(numClfs)+k];
			double xmax = ROI[2*(numClfs)+k]+xmin;
			double ymin = ROI[1*(numClfs)+k];
			double ymax = ROI[3*(numClfs)+k]+ymin;

			const int maxr = (int) ceil(float(ymax-modelHt+1)/stride);
			const int minr = (int) ceil(float(ymin)/stride);
			const int maxc = (int) ceil(float(xmax-modelWd+1)/stride);
			const int minc = (int) ceil(float(xmin)/stride);
			*/
		mxArray *trees = mxGetCell(clfs,k);
		...
		const float cascThr = (float) mxGetScalar(mxGetField(trees,0,"cascThr"));
		...
		for( int c=0; c<width1; c++ ) for( int r=0; r<height1; r++ ) {
			...
			if(h>cascThr) {
				...
				hs1.push_back(h); id.push_back(k+1);
			}
		}
	}
	...
	for( int i=0; i<m; i++ ) {
		...
		bbs[i+4*m]=hs1[i]; bbs[i+5*m]=id[i];
	}
}
