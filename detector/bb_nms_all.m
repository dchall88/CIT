function [bbs,ind] = bb_nms_all( bbs, varargin )
...
assert(maxn>=2); assert(numel(overlap)==1); ind=[];

% discard bbs below threshold and run nms1
if(isempty(bbs)), bbs=zeros(0,5+separate); end; if(strcmp(type,'none')), return; end
kp=bbs(:,5)>thr; bbs=bbs(kp,:); if(isempty(bbs)), ind=kp'; return; end
if(~isempty(resize)), bbs=bbApply('resize',bbs,resize{:}); end
pNms1={type,thr,maxn,radii,overlap,0};
if(~separate || size(bbs,2)<6), [bbs,ind]=nms1(bbs,pNms1{:}); else
    ts=unique(bbs(:,6)); m=length(ts); bbs1=cell(1,m); ind=cell(1,m);
    for t=1:m,
        if(t > 1), pNms1{1}='maxid'; end
        [bbs1{t},ind{t}]=nms1(bbs(bbs(:,6)==ts(t),:),pNms1{:});
        ind1=find(bbs(:,6)==ts(t)); ind{t}=ind1(ind{t});
    end
    bbs=cat(1,bbs1{:}); ind=cat(1,ind{:});
end

    function [bbs,ind] = nms1( bbs, type, thr, maxn, radii, overlap, isy )
        ...
        switch type
            case 'max', [bbs,ind] = nmsMax(bbs,overlap,0,ovrDnm);
            case 'maxg', [bbs,ind] = nmsMax(bbs,overlap,1,ovrDnm);
            case 'maxid', [bbs,ind] = nmsMaxIndv(bbs);
    ...
    function [bbs,ind] = nmsMaxIndv( bbs )
        [~,ind]=max(bbs(:,5)); bbs=bbs(ind,:);
    end

    function [bbs,ind] = nmsMax( bbs, overlap, greedy, ovrDnm )
